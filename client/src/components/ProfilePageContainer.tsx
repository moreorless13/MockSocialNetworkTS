...
const ProfilePageContainer = ({userId, username, posts, following, followers}: any) => {
    const [key, setKey] = useState('Posts');

    return (
        <Tabs id='controlled-tabs' activeKey={key} onSelect={(key: any) => setKey(key)} className='mb-3'>
            <Tab eventKey='Posts' title='Posts'><UsersPostsTab userId={userId} username={username} posts={posts} /> </Tab>
